...
# Include costs in buying-power validation, not just in accounting after approval.
text = text.replace('            if cash < notional:\n                return {"ok": False, "message": "Not enough paper cash for the approved long."}', '            if cash < notional + entry_cost:\n                return {"ok": False, "message": "Not enough paper cash for the approved long including simulated costs."}', 1)
...
# Correct legacy r36 display P&L so it includes both entry and exit costs.
text = text.replace('realized = (price - entry_price) * qty - exit_cost', 'entry_cost_est = execution_cost_bps(qty * entry_price)\n            realized = (price - entry_price) * qty - entry_cost_est - exit_cost')
text = text.replace('realized = (entry_price - price) * qty - exit_cost', 'entry_cost_est = execution_cost_bps(qty * entry_price)\n            realized = (entry_price - price) * qty - entry_cost_est - exit_cost')
text = text.replace('f"{reason} | realized_pnl=${realized:,.2f} | exit_cost=${exit_cost:,.2f}"', 'f"{reason} | realized_pnl=${realized:,.2f} | entry_cost=${entry_cost_est:,.2f} | exit_cost=${exit_cost:,.2f}"')

text = re.sub(r'APP_VERSION = "[^"]+"', 'APP_VERSION = "2026.09.04-r37-reliability-v31"', text, count=1)
...
require('entry_cost_est = execution_cost_bps(qty * entry_price)' in text, "round-trip cost accounting missing")
