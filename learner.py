...
def market():
    ...
    for x in m:
        try:
            exp=pd.Timestamp(x.get('close_time') or x.get('expiration_time')).timestamp(); strike=float(x.get('floor_strike') or x.get('cap_strike'))
            if exp>now:out.append((exp,x,strike))
        except Exception:pass
    if not out:return None
    exp,x,strike=sorted(out,key=lambda z:z[0])[0]
    vals=[]
    for k in ['yes_bid_dollars','yes_ask_dollars']:
        try:vals.append(float(x[k]))
        except Exception:pass
    return {'ticker':x.get('ticker',''),'expires_at':exp,'target':strike,'prob':sum(vals)/len(vals) if vals else .5}

def calls(d,m):
    z=d.iloc[-1]; p=float(z.close); r3=p/float(d.close.iloc[-4])-1; r8=p/float(d.close.iloc[-9])-1; r15=p/float(d.close.iloc[-16])-1
    trend=.55*np.sign(z.ema9-z.ema21)+.45*np.sign(z.ema21-z.ema50); mom=np.clip((float(z.rsi)-50)/30,-1,1); vol=np.clip(np.sign(z.close-z.open)*abs(float(z.close-z.open))/max(float(z.high-z.low),1),-1,1)
    ...
    base={'Trend AI':trend,'Momentum AI':mom,'Volume AI':vol,'Pattern AI':vol,'Support/Resistance AI':sr,'Volatility AI':-np.clip((p-float(d.close.tail(20).mean()))/max(float(d.close.tail(20).std())*3,1),-1,1),'Market Regime AI':regime,'Whale AI':0.0,'Liquidity AI':0.0,'Derivatives AI':0.0,'Event AI':event,'Historical Pattern AI':np.clip(r3/.003,-1,1)}
...
def register(s,d,m):
    ...
    p=float(d.close.iloc[-1]); bot,rets=calls(d,m); f=s['forecast']; directional=f['w_ret3']*rets[0]+f['w_ret8']*rets[1]+f['w_ret15']*rets[2]+f['bias']; move=p*np.clip(directional,-.012,.012)*f['momentum_scale']; move+=np.clip((m['target']-p)*f['target_influence'],-p*.003,p*.003); pred=p+move
