...
This evaluator uses only information available at each historical timestamp,
grades each specialist on the next 15-minute direction, and records both global
and market-regime performance. It intentionally excludes specialists whose real
inputs cannot be reconstructed faithfully from OHLCV archives (Whale, Liquidity,
Derivatives, Kalshi Context, Combination).
"""
...
def fetch_month(year, month):
    req = Request(month_url(year, month), headers={"User-Agent": "BTC-AI-Specialist-History-v7/1.0"})
    ...
    return df.dropna(subset=["time", "open", "high", "low", "close", "volume"])[["time", "open", "high", "low", "close", "volume"]]
...
def main():
    ...
    for year, month in month_iter(start_year, start_month, end_year, end_month):
        ...
        if not history.empty:
            month_df = pd.concat([history.tail(180), month_df], ignore_index=True).drop_duplicates("time").sort_values("time")

        enriched = enrich_history_core(month_df.copy())
        for i in range(len(enriched)):
            ...
            while pending and pending[0][0] <= ts.value:
                _, start_px, regime, calls = pending.popleft()
            ...
            pending.append((ts.value + 15 * 60 * 1_000_000_000, px, regime, calls))

        history = month_df.tail(240).copy()
    ...
    report = {
        ...
        "excluded_specialists": ["Whale AI", "Liquidity AI", "Derivatives AI", "Kalshi Context AI", "Combination AI"],
    ...
    print(json.dumps({"evaluated_windows": total_windows, "months_loaded": len(months_loaded)}, indent=2))
