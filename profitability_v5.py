...
def lifetime_directional_accuracy(db_path=None):
    ...
    try:
        with sqlite3.connect(path, timeout=1.0) as conn:
            row = conn.execute(
                """SELECT SUM(CASE WHEN correct=1 THEN 1 ELSE 0 END),
                          SUM(CASE WHEN correct=0 THEN 1 ELSE 0 END)
                   FROM predictions
                   WHERE resolved=1 AND correct IS NOT NULL
...
def latest_kalshi_call_entry(db_path=None):
    ...
    try:
        with sqlite3.connect(path, timeout=1.0) as conn:
            ...
            row = conn.execute(
                "SELECT side, entry_price FROM kalshi_paper_positions "
                "WHERE entry_price IS NOT NULL ORDER BY opened_at DESC, id DESC LIMIT 1"
            ).fetchone()
        if not row:
            return None, None
...
def _install_dashboard_adapter():
    ...
    # v5 fixes the Backtest runner on Streamlit builds where the public st.tabs
    ...
    original_metric = getattr(
        DeltaGenerator, "_btc_profitability_original_metric", DeltaGenerator.metric
    )
    original_markdown = getattr(
        DeltaGenerator, "_btc_profitability_original_markdown", DeltaGenerator.markdown
    )
    original_tabs = getattr(
        DeltaGenerator, "_btc_profitability_original_tabs", DeltaGenerator.tabs
    )
    original_st_tabs = getattr(
        st, "_btc_profitability_original_tabs", st.tabs
    )
    ...
    st._btc_profitability_original_tabs = original_st_tabs
    ...
    def markdown_adapter(self, body, *args, **kwargs):
        ...
        if isinstance(rendered, str) and _profitability_hold_active(st):
            rendered = rendered.replace(">SCALP UP<", ">HOLD SCALP UP<").replace(
                ">SCALP DOWN<", ">HOLD SCALP DOWN<"
            )
        return original_markdown(self, rendered, *args, **kwargs)

    def _inject_runner(labels, containers):
        try:
            backtest_idx = next(
                (
                    i
                    for i, label in enumerate(labels)
                    if str(label).strip().lower() == "backtest"
                ),
                None,
            )
            if backtest_idx is None or backtest_idx >= len(containers):
                return
    ...
    def tabs_adapter(self, tabs, *args, **kwargs):
        labels = list(tabs)
        containers = original_tabs(self, labels, *args, **kwargs)
        _inject_runner(labels, containers)
        return containers

    def public_tabs_adapter(tabs, *args, **kwargs):
        labels = list(tabs)
        containers = original_st_tabs(labels, *args, **kwargs)
        _inject_runner(labels, containers)
        return containers
