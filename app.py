Warning: truncated output (original token count: 59677)
Total output lines: 6353
...
def rolling_specialist_accuracy(window):
    ...
    if isinstance(remote, dict):
        ...
        if isinstance(histories, dict) and histories:
            out = []
            for specialist,…9677 tokens truncated…se:
    st.markdown(
        """
        <style>
...
def live_dashboard():
    ...
    with tab_ai:
        ...
        try:
            ...
            render_research_dashboard(_bot_learning_state)
        ...
        for r in results.values():
            rows.append({
                ...
                "Reason": r["reason"],
                "Counterargument": r.get("counterargument", ""),
